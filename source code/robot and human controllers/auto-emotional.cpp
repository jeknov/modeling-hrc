...
void MyController::onInit(InitEvent &evt)
{
	...
	task_end = false;
				...
				if((fp2 = fopen("happy.txt", "r")) == NULL) {
					printf("File happy.txt does not exist.\n");
					exit(0);
				}
				else{
					printf("It's a happy robot.\n");
					fscanf(fp2, "%d", &happyNum);
					fscanf(fp2, "%d", &sleeptime);
...
printf("happy value  %d \n",happy_msg );
	...
	if(!end && start){
		...
		if (happy_msg==1){
				printf("happy_msg==true.\n");


				//stop = true;

				for(int j=0; j<happyNum; j++){
					 //addx += dx;
					 //addz += dz;
					usleep(sleeptime);
					//my->setPosition(pos.x()+addx, s_HEIGHT[i], pos.z()+addz);
					my->setJointAngle("HEAD_JOINT1", DEG2RAD(s_HEAD_JOINT1[j]));
					my->setJointAngle("LARM_JOINT1", DEG2RAD(s_LARM_JOINT1[j]));
					my->setJointAngle("LARM_JOINT4", DEG2RAD(-s_LARM_JOINT4[j]));
					my->setJointAngle("RARM_JOINT1", DEG2RAD(s_RARM_JOINT1[j]));
					my->setJointAngle("RARM_JOINT4", DEG2RAD(-s_RARM_JOINT4[j]));
					my->setJointAngle("WAIST_JOINT0", DEG2RAD(s_WAIST_JOINT0[j]));
				}
					my->setJointAngle("HEAD_JOINT1", 0);
					my->setJointAngle("WAIST_JOINT0",0);
					my->setJointAngle("LARM_JOINT4", 0);
					my->setJointAngle("RARM_JOINT4", 0);
				happy_msg = 0;

				//initCondition();
				printf("Stop being happy.\n");
		...
		if(checkPoint==END){
			...
			if(graspObjectName == "curry" || graspObjectName == "spagetti"){//T4
               ...
               happy_msg = 1;
               printf("happy value f1  %d \n",happy_msg );
				initCondition();
               printf("happy value f2  %d \n",happy_msg );
				moveObject->setPosition( 20.0, 68.5, 55.0 );
				printf("happy value f3  %d \n",happy_msg );
				happy_msg = 1;
				printf("happy value f4  %d \n",happy_msg );
			}
